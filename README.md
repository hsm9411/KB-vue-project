...
### 5. Base Components 구축을 통한 디자인 시스템 구현 (Phase 3)
- **Component-Driven Development (CDD):** `BaseButton`, `BaseInput`, `BaseCard` 등 재사용 가능한 UI 원자(Atoms)를 먼저 구축하고 이를 조립하여 화면을 구성하는 방식.
- **Vue 3 컴포넌트 통신 기법:**
    - `props`: 부모로부터 데이터 및 스타일 옵션(`variant`, `size`) 수신.
    - `slots`: `<slot />`을 활용한 유연한 컨텐츠 주입 (Card 컴포넌트 등).
    - `v-model` 연동: 커스텀 입력 컴포넌트와 양방향 데이터 바인딩 구현.
- **디자인 일관성:** 모든 페이지의 버튼 호버 효과, 입력창 포커스 스타일, 카드 그림자 등을 한곳에서 관리하여 앱 전체의 완성도를 극적으로 향상.

### 6. Chart.js를 활용한 데이터 시각화 (Phase 4)
- **차트 통합:** `vue-chartjs`를 활용하여 수입/지출 추이(막대 그래프)와 지출 비중(도넛 차트)을 시각화하는 방법 학습.
- **반응형 캔버스:** 브라우저 크기에 따라 유연하게 크기가 조절되는 차트 컨테이너 설계.

### 7. 복잡한 비즈니스 로직 및 다중 스토어 연동
- **N빵 연산 로직:** `TransactionForm`에서 실시간으로 1인당 예상 금액을 계산하고, 원금과 정산금을 구분하여 저장하는 로직 구현.
- **그룹 데이터 매핑:** `txStore`, `userStore`, `groupStore` 등 여러 스토어의 데이터를 조합하여 실시간 '소비 레이스' 순위를 도출하는 연산 기법 체득.
- **Navigation Guard:** 로그인 여부에 따라 비공개 경로 접근을 차단하는 보안 라우팅 처리.
